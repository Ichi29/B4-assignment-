...
if __name__ == "__main__":
    ...
    output_dir = "experiment_results_angle"
    ...
    for fov_x, fov_y in fov_conditions:
        ...
        save_image(
            output_dir,
            f"fov_x{fov_x}_y{fov_y}_theta0_phi0.jpg",
            out
        )

    # ============================================================
    # 実験2：視線方向の変更（左右方向）
    # 画角と出力画像サイズを固定し，theta_eye_deg のみを変更する
    # ============================================================
    theta_conditions = [-60, 0, 60]

    for theta in theta_conditions:
        out = equirectangular_to_perspective_xy_rotation(
            img,
            fov_x_deg=60,
            fov_y_deg=40,
        ...
        save_image(
            output_dir,
            f"view_theta{theta}_phi0_fov60_40.jpg",
            out
        )

    # ============================================================
    # 実験2：視線方向の変更（上下方向）
    # 画角と出力画像サイズを固定し，phi_eye_deg のみを変更する
    # ============================================================
    phi_conditions = [-30, 0, 30]
    ...
    for phi in phi_conditions:
        ...
        save_image(
            output_dir,
            f"view_theta0_phi{phi}_fov60_40.jpg",
            out
        )

    # ============================================================
    # 実験3：回転方法の違いで使う角度指定画像
    # assignment_2_3d_vector_experiment.py の vector_theta30_phi20 と比較する
    # ============================================================
    out = equirectangular_to_perspective_xy_rotation(
        img,
        fov_x_deg=60,
    ...
    save_image(
        output_dir,
        "rotation_method_angle_theta30_phi20_fov60_40.jpg",
        out
    )

    print("\n角度指定による実験画像の生成が完了しました。")
